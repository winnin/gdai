...
class DocumentChunk(BaseModel):
    ...
    def validate_chunk(cls, value, _: ValidationInfo):
        if not value or not value.strip():
            raise ValueError("chunk document_id can't be empty")
        return value
    ...
    def validate_document_id(cls, v):
        if v is None or (isinstance(v, str) and not v.strip() and len(v) == 0):
    ...
    def validate_tenant_id(cls, v):
        if v is None or (isinstance(v, str) and not v.strip() and len(v) == 0):
            raise ValueError("tenant_id can't be empty")
        return v
