...
from src.repositories.models import DocumentChunkModel, DocumentModel, DocumentStatusEnum, DocumentTypeEnum, QueryModel
        ...
        try:
            async with SessionLocal() as session:
                db_doc = DocumentModel(
                    ...
                    tenant_id=tenant_id,
                    name=document.name,
                    status=DocumentStatusEnum[document.status],
        ...
        try:
            async with SessionLocal() as session:
                ...
                if db_doc:
                    ...
                    db_doc.updated_at = document.updated_at
                    await session.commit()
...
class PGVectorDocumentChunkRepository(DocumentChunkRepository):
        ...
        try:
            async with SessionLocal() as session:
                db_chunk = DocumentChunkModel(
                    ...
                    fk_document_id=chunk.document_id,
    ...
    async def get_by_document_id(self, tenant_id: str, document_id: str):
        async with SessionLocal() as session:
            result = await session.execute(select(DocumentChunkModel).where(DocumentChunkModel.tenant_id == tenant_id, DocumentChunkModel.fk_document_id == document_id))
            chunks = result.scalars().all()
            return [DocumentChunk.model_validate(chunk) for chunk in chunks]
